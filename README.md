...
`       ，        
下载完成后输入`bash reinstall.sh ...`可查看可安装系统，以安装debian13 为例 指令为 ： `bash reinstall.sh debian 13`           ，输入新的密码（同ssh登陆密码） 并再次输入确认  ，最后输入`reboot`重启vps，断开shh连接重新连接，这里密钥已经发生改变，需要删除原来的本地密钥后再连接     ，然后就可以开始部署节点了。      
